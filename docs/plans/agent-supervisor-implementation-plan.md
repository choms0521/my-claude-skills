# Agent Supervisor — 구현 상세 계획서

> 설계 명세(`docs/claude-agent-supervisor-spec.md`)를 기준으로, 다른 개발자가 이 문서만 보고 곧바로 구현에 착수할 수 있도록 작성한 단일 통합 계획서다.
...
| 구분 | 위치 | 설명 |
|---|---|---|
| **편집(authoring)** | `skills/agent-supervisor/` (이 레포) | 정본. 여기만 손댄다. |
| 생성물(build 산출) | `generated/{claude,codex}/skills/agent-supervisor/` | `build-skills.mjs`가 생성. 직접 수정 금지. |
| Claude 노출 | `.claude/skills/agent-supervisor/SKILL.md` | generated를 가리킴. |
| **런타임 산출물** | 사용자 머신의 user-level 경로 | 스킬 *실행 결과*가 안착하는 곳(아래) |

런타임 산출물(스킬을 실제로 돌렸을 때 사용자 머신에 생기는 것):
- 심사관 서브에이전트: `~/.claude/agents/agent-supervisor-judge.md` (스킬이 첫 실행 시 멱등 설치 — §4.2)
- 전역 지침 출력: `~/.claude/rules/agent-discipline.md` (자동 전역 로드)
- 진단 이력: `~/.claude/.agent-supervisor/history/<ISO>.json`
...
- **목표**: 최근 Claude Code 세션 기록을 진단하여 행동 문제(예절·규율)를 등급화하고, 문답으로 합의한 규칙을 `~/.claude/rules/agent-discipline.md`에 고정하는 온디맨드 스킬 `/agent-supervisor`.
...
| 4 | 스킬 이름 | `/agent-supervisor` |
| 5 | 점수 표현 | 등급 S/A/B/C/D + 사례 인용 |
| 6 | 심사관 | 커스텀 `agent-supervisor-judge` 서브에이전트(제3자 전송 없음) |
...
├── agents/
│   └── agent-supervisor-judge.md       # 심사관 서브에이전트 원본(런타임에 ~/.claude/agents/로 설치)
├── scripts/
...
│   ├── install-judge.mjs      # ~/.claude/agents/agent-supervisor-judge.md 멱등 설치
...
```json
{
  ...
  "source": { "claude": ".claude/skills/agent-supervisor/SKILL.md" },
  "assets": ["scripts", "agents/agent-supervisor-judge.md", "fixtures/known-violation.json"],
...
### 4.2 `agent-supervisor-judge` 서브에이전트 — 배치·중립화·캘리브레이션
...
**배치(스킬은 번들 불가 — 공식 확인)**: 스킬은 발견 가능한 서브에이전트를 ship할 수 없다. Claude Code는 `.claude/agents/`·`~/.claude/agents/`만 스캔한다. 그러므로 번들 원본(`skills/agent-supervisor/agents/agent-supervisor-judge.md`)을 두되, 스킬 첫 실행 시 `install-judge.mjs`가 `~/.claude/agents/agent-supervisor-judge.md`로 **멱등 설치**한다(관리 마커로 버전 추적, 사용자 수동 편집 보존).

**`agent-supervisor-judge.md` 원본(시스템 프롬프트) 골자**:
...
```
saveHistory(diagnosis, home) -> path   // ~/.claude/.agent-supervisor/history/<ISO>.json
...
mergeIntoDiscipline(home, composedRules)
  // ~/.claude/rules/agent-discipline.md 의 <!-- agent-supervisor:start --> ~ :end --> 구획만 교체
...
```
/agent-supervisor [--days 7|3] [--scope global|current]
 1. install-judge.mjs  : ~/.claude/agents/agent-supervisor-judge.md 멱등 설치
 2. collect.mjs        : 추출 + 선마스킹 + tool_result 절삭 -> JSON
 3. (캘리브레이션)      : fixtures/known-violation.json 로 심사관 자가검증 -> passed 확인
 4. evaluate           : agent-supervisor-judge 서브에이전트에 트랜스크립트 위임 -> 등급 JSON
...
- 정본은 `skills/agent-supervisor/`만. `generated/*`, `.claude/skills/*/SKILL.md` 직접 수정 금지.
- 보조 자산(`scripts`, `agents/agent-supervisor-judge.md`, `fixtures/...`)은 `skill.json`의 `assets`에 선언.
...
### Step 0 — 스캐폴딩
- 산출물: `skills/agent-supervisor/{skill.json, common.md, adapters/claude.md, adapters/codex.md}` 골격.
- 종료조건: `test -f skills/agent-supervisor/skill.json && node -e "JSON.parse(require('fs').readFileSync('skills/agent-supervisor/skill.json','utf8'))"` → exit 0.

### Step 1 — collect.mjs (수집·마스킹·절삭)
- 산출물: `scripts/collect.mjs` + `lib/{mask,filter,truncate}.mjs` + `tests/*.test.mjs`.
- 종료조건:
  - `node --test skills/agent-supervisor/tests/` → exit 0 (마스킹/필터/절삭 단위 테스트 통과).
...
### Step 2 — agent-supervisor-judge 서브에이전트 + 멱등 설치 + 캘리브레이션
- 산출물: `agents/agent-supervisor-judge.md`, `scripts/install-judge.mjs`, `fixtures/known-violation.json`.
- 종료조건:
  - 멱등 설치: `install-judge.mjs`를 2회 실행 후 `~/.claude/agents/agent-supervisor-judge.md`가 1개만 존재하고 내용 동일(`diff` → 변화 없음).
...
- 종료조건: 2회 실행 후 `~/.claude/.agent-supervisor/history/`에 2개 파일 존재하고, 2회차 출력에 `improved|worsened|new|resolved` 4개 키가 모두 포함됨(`node -e` 검증 exit 0).
...
- 종료조건:
  - 멱등: 같은 입력으로 2회 실행 후 `agent-discipline.md`의 `<!-- agent-supervisor:start -->` 마커가 정확히 1쌍(`grep -c 'agent-supervisor:start'` → `1`).
...
- 종료조건(모두 exit 0):
  - `node tools/build-skills.mjs agent-supervisor`
  - `node tools/validate-skills.mjs agent-supervisor`
  - (Codex 로컬 배포가 필요하면) `node tools/sync-codex-project-skills.mjs agent-supervisor`
  - README에 agent-supervisor 항목 추가(`grep -c 'agent-supervisor' README.md` ≥ 1) — `pr-workflow.md` 요구.
  - `adapters/codex.md`에 Claude 전용·미지원 사유 명시(`grep -ciE 'codex|unsupported|미지원' skills/agent-supervisor/adapters/codex.md` ≥ 1).
...
- 심사관: `~/.claude/agents/agent-supervisor-judge.md`
- 출력 규칙: `~/.claude/rules/agent-discipline.md`
- 이력: `~/.claude/.agent-supervisor/history/<ISO>.json`
