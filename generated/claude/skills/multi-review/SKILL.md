...
- Install target: `~/.claude/skills/multi-review`
- Support level: full
...
- `agy`(Antigravity CLI)가 있으면 Gemini 관점 리뷰를 외부 프로세스로 위임할 수 있습니다. 구 `gemini` CLI는 2026-06-18 서비스 종료되어 `agy`로 대체되었습니다. 레거시 `gemini`가 아직 남아 있는 환경이면 그대로 써도 됩니다.
- 외부 CLI나 네이티브 하위 에이전트가 없어도, 현재 세션에서 **서로 다른 관점으로 3번 독립 리뷰**를 수행해 동일한 출력 파일을 만들어야 합니다.

- Claude 런타임에서는 OMC, `agy`(Antigravity CLI) 연동, Claude 하위 에이전트를 사용할 수 있습니다.
...
| `agy` 가 작업트리(추적 파일) 수정 | 사전 git 스냅샷으로 즉시 되돌림 |
