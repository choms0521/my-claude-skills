...
- Runtime: Claude Code
- Invocation: `/agent-supervisor`
- Install target: `~/.claude/skills/agent-supervisor`
- Support level: full
- Canonical source: `skills/<name>/...`에서 생성된 Claude용 어댑터입니다.
- Runtime notes: 이 스킬은 Claude Code 세션 JSONL 기록(`~/.claude/projects/**/*.jsonl`)을 읽고, `~/.claude/agents/agent-supervisor-judge.md` 심사관 서브에이전트를 멱등 설치하며, 진단 결과를 `~/.claude/rules/agent-discipline.md`에 병합합니다. 진단 이력은 `~/.claude/.agent-supervisor/history/`에 저장됩니다.
...
```bash
SKILL_DIR="$HOME/.claude/skills/agent-supervisor"
...
| 심사관 서브에이전트 | `$HOME/.claude/agents/agent-supervisor-judge.md` |
| 전역 지침(출력) | `$HOME/.claude/rules/agent-discipline.md` |
| 진단 이력 | `$HOME/.claude/.agent-supervisor/history/<ISO>.json` |
...
수집·마스킹·절삭된 트랜스크립트를 `agent-supervisor-judge` 서브에이전트에 위임합니다.

- 위임 방식: 메인 에이전트가 `subagent_type: agent-supervisor-judge`로 위임하거나 `context: fork` + `agent: agent-supervisor-judge`.
...
- `$HOME/.claude/rules/agent-discipline.md`의 `<!-- agent-supervisor:start -->` ~ `<!-- agent-supervisor:end -->`
...
`$HOME/.claude/.agent-supervisor/history/<ISO>.json`에 이번 진단을 저장합니다.
