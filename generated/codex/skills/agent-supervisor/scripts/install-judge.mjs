#!/usr/bin/env node
//
// install-judge.mjs — idempotently install the agent-supervisor-judge subagent into the
...
const SOURCE_AGENT = path.join(SCRIPT_DIR, '..', 'agents', 'agent-supervisor-judge.md');
const MARKER_RE = /<!--\s*agent-supervisor:judge-version:\s*([a-f0-9]+)\s*-->/;
...
function buildInstalledContent(sourceText, hash) {
  return `${sourceText.trimEnd()}\n\n<!-- agent-supervisor:judge-version: ${hash} -->\n`;
}
...
export async function installJudge({ home }) {
  ...
  const targetPath = path.join(targetDir, 'agent-supervisor-judge.md');
