...
runtime: codex
support-level: none
generated-from: skills/conduct-coach
---
...
conduct-coach는 Claude Code 전용 스킬이며 Codex 런타임에서는 동작하지 않습니다(미지원). 다음 전제에 의존하기 때문입니다.
...
- **심사관 서브에이전트**: 진단은 `~/.claude/agents/conduct-judge.md`로 설치되는 커스텀 Claude Code 서브에이전트(시스템 프롬프트 + read-only 도구)에 위임됩니다. Codex에는 이 서브에이전트 발견/위임 기전이 없습니다.
