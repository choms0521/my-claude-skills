...
runtime: codex
support-level: full
generated-from: skills/check-github-copilot-review
---
...
- Runtime: Codex
- Invocation: `$check-github-copilot-review`
- Install target: `$CODEX_HOME/skills/check-github-copilot-review` (기본값: `~/.codex/skills/check-github-copilot-review`)
- Support level: full
...
Codex MUST follow this workflow exactly when this skill is invoked.
...
코멘트가 없는 경우:
- **사이클 1**: "Copilot 리뷰 코멘트가 없습니다"를 출력하고 종료합니다.
- **사이클 2+**: 이전 사이클에서 FIX 후 푸시했으므로 Copilot이 아직 리뷰 중일 수 있습니다. Stage 4 조건 B의 폴링 절차(30초 간격, 최대 10분)를 즉시 시작합니다. 폴링 중 새 코멘트가 발견되면 Stage 2로 진행하고, 10분 경과 후에도 없으면 사용자에게 계속 여부를 묻습니다.
...
```
## Copilot 리뷰 코멘트 — 사이클 {cycle}/3 ({N}건, 신규 {new}건, 이전 보류 재검토 {hold}건)
...
> **판단 원칙 (CRITICAL):** Copilot의 지적은 확정된 결함이 아니라 **검증 대상 제안**입니다.
> 기본값은 수용이 아니라 검증입니다. 코드를 직접 읽어 지적이 실제로 성립함을 확인한 항목만 FIX로 분류하고,
> 성립하지 않음이 확인된 항목만 DISMISS로 분류하며, FIX·DISMISS 어느 쪽도 근거로 확정하지 못한 항목은 HOLD가 기본값입니다(아래 HOLD 절차).
> "리뷰 봇이 지적했으니 고친다"는 이유만으로 FIX를 선택하지 않습니다.

각 코멘트에 대해:
1. 해당 파일의 관련 코드를 읽습니다 (Read tool 사용)
2. Copilot의 지적이 **실제로 성립하는지 근거를 확인합니다**. 파일 하나만 보고 판단하지 않으며, 필요하면 호출부·상위 계층·기존 테스트·프로젝트 컨벤션까지 추적합니다 (예: "에러 핸들링 누락" 지적 → 상위 미들웨어나 호출부에서 이미 처리 중인지 확인)
3. 아래 기준으로 **수정 필요(FIX)** 또는 **불필요(DISMISS)** 를 판단합니다

**FIX 판단 기준** — 아래 중 하나에 해당하고, 2번에서 실제로 성립함을 확인한 경우에만 FIX로 분류합니다:
- 실제 버그나 논리 오류가 있는 경우
- 보안 취약점이 존재하는 경우
- 실패 경로가 처리되지 않아 에러 핸들링이 누락된 경우
- 성능 문제가 구체적으로 특정되는 경우 (어떤 입력·규모에서 어떻게 저하되는지 설명 가능)
- 명세·문서·타입 선언과 구현이 불일치하는 경우

> 위 다섯 가지는 모두 **실제 결함**을 가리는 관문입니다. 어느 하나에도 해당하지 않으면 FIX가 아닙니다.
> "코드 품질이 좋아진다", "이 편이 더 낫다" 같은 취향 판단은 단독 FIX 사유가 되지 않습니다.

**DISMISS 판단 기준:**
- 이미 다른 곳에서 처리되고 있는 경우
- 프로젝트 컨벤션에 맞는 의도적 패턴인 경우
- 오탐(false positive)인 경우
- 동작 결함이 아닌 스타일·네이밍·리팩터링 취향 제안인 경우
- 수정 시 오히려 가독성이 떨어지거나 불필요한 복잡성이 추가되는 경우
- 컨텍스트를 고려하면 현재 코드가 적절한 경우
- 해당 PR의 변경 범위를 벗어난 기존 코드에 대한 제안인 경우

**판단이 확실하지 않은 경우 (HOLD):**

FIX와 DISMISS 어느 쪽도 근거로 확정할 수 없으면 **임의로 수정하지 않습니다**. 해당 항목을 HOLD로 표시하고, 무엇을 확인하지 못했는지 명시하여 사용자에게 판단을 요청합니다:

```
❓ 판단 보류 — {file}:{line}
Copilot 지적: {요약}
확인한 것: {확인된 사실}
확정하지 못한 것: {판단에 필요한데 확인 못 한 지점}

FIX / DISMISS 중 어느 쪽으로 처리할까요?
```

- 사용자가 응답하면 그에 따라 FIX 또는 DISMISS로 확정하여 처리합니다.
- 사용자가 판단을 미루면 HOLD로 남깁니다. HOLD 항목은 **코드를 수정하지 않고, 댓글도 resolve도 하지 않으며, `processed_comment_ids`에도 추가하지 않습니다** (다음 사이클에서 다시 검토 대상이 됩니다).
...
| # | 파일 | 확정하지 못한 지점 |
|---|------|--------------------|
| 1 | src/db.ts:88 | 동시 호출 여부를 확인할 수 없어 경합 발생 가능성 미확정 |
...
```
...
**조건 A: FIX 0건 (코드 변경 없음)**

폴링 없이 바로 종료하되, HOLD 잔여 여부에 따라 보고를 구분합니다.

**A-1. HOLD 0건** — 모든 코멘트를 FIX 또는 DISMISS로 확정 처리한 경우:
...
**A-2. HOLD 1건 이상** — 확정하지 못한 항목이 남아 있으므로 **완료로 보고하지 않습니다**:

```
### 보류 항목 {N}건이 남아 있습니다

확정하지 못한 {N}건은 코드 수정·댓글·resolve 없이 그대로 두었습니다.
아래 항목의 판단을 알려주시면 이어서 처리하며, 판단 없이 종료해도
재실행 시 다시 검토 대상이 됩니다.
...
2. **폴링 시작**: 30초 간격으로 최대 10분(20회) 동안 새 Copilot 리뷰 코멘트를 확인합니다
   ```
   ### Copilot 재리뷰 대기 중...

   코드가 푸시되었습니다. Copilot에 리뷰 요청을 시도했습니다. 30초 간격으로 확인합니다 (최대 10분).
   ```
...
5. **리뷰 완료 + 코멘트 없음**: Copilot이 리뷰를 완료했지만 추가 코멘트가 없는 경우 즉시 종료합니다:
   ...
   ```
6. **10분 경과, 새 코멘트 없음**: 사용자에게 계속 여부를 묻습니다
   ```
   ⏰ 10분간 새로운 Copilot 코멘트가 발견되지 않았습니다.
...
3. 이번 사이클에서 **FIX 또는 DISMISS로 확정 처리한** comment_id들을 `processed_comment_ids` 배열에 추가합니다. **HOLD 항목의 comment_id는 추가하지 않습니다** (다음 사이클에서 다시 검토 대상이 되어야 하므로)
4. `history` 배열에 이번 사이클 결과 기록 (`hold`는 0건이면 생략 가능)
...
```json
{
  ...
  "history": [
    {"cycle": 1, "fix": 3, "dismiss": 1, "at": "2026-04-01T12:00:00Z"},
    {"cycle": 2, "fix": 2, "dismiss": 0, "hold": 1, "at": "2026-04-01T12:30:00Z"}
  ]
}
```
...
| 지적 성립 여부를 확정할 수 없음 | HOLD로 표시, 사용자 확인 요청. 코드 수정·댓글·resolve 모두 하지 않음 |
