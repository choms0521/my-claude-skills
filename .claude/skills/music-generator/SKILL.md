...
| 패턴 | 예시 |
|------|------|
| `아티스트 - 곡명` | "IU - Blueming", "BTS - Dynamite" |
| `아티스트의 곡명` | "아이유의 블루밍" |
| `곡명 + 느낌/스타일/같은` | "Blueming 느낌", "Dynamite 같은" |
| `아티스트 + 스타일/느낌` | "아이유 스타일", "뉴진스 느낌" |
...
- 장르(--genre): Disco Pop ← 참고곡 분석
- 분위기(--mood): bright, energetic, retro ← 참고곡 분석
- BPM(--bpm): 114 ← 참고곡 분석
- 보컬(--vocals): bright male tenor ← 사용자 선택
- 악기(--instruments): funk guitar, bass, brass, synth ← 참고곡 분석
...
- `--references`는 **텍스트 설명만 지원**합니다 (예: "similar to IU", "BTS - Dynamite 같은 느낌"). YouTube URL이나 오디오 파일 업로드는 지원하지 않습니다. Interview Mode에서는 곡명/아티스트 입력 시 WebSearch로 상세 정보를 자동 수집하여 `--prompt`, `--genre`, `--mood` 등에 반영합니다 (상세는 "Reference Song Analysis" 섹션 참조)
