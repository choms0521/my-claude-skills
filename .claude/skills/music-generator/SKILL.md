...
**선택지:**
- **Easy 모드** (초보자 친화): 직관적 질문 6개 (instrumental은 4개), 힌트 풍부, 전문 용어 최소화
- **Expert 모드** (상세 설정): BPM, Key 등 전문적 옵션 10개 직접 입력
...
- **YouTube 링크 선택지**: Pre-Check에서 `which gemini` 성공 시, AskUserQuestion 선택지에 "YouTube 링크로 참고곡 분석" 옵션을 추가. 사용자가 이 옵션을 선택하면 YouTube URL 입력을 받아 **YouTube Audio Analysis** 절차를 실행
...
```bash
which yt-dlp  # 필수 — 없으면 "yt-dlp가 필요합니다. brew install yt-dlp로 설치해주세요." 안내 후 WebSearch fallback
```
...
`trap`에 의해 자동 정리됩니다. 분석 완료 후 MP3 파일은 즉시 삭제됩니다.
...
- 참고(--references): https://youtube.com/watch?v=xxx (YouTube Audio Analysis)
</details>
```
...
- `--references`는 **텍스트 설명만 지원**합니다 (예: "similar to IU", "BTS - Dynamite 같은 느낌"). Interview Mode에서 참고곡 분석은 두 가지 방식을 지원합니다:
