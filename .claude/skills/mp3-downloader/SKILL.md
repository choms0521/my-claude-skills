...
# 커스텀 경로 지정
bash scripts/download_mp3.sh --out "~/Music/downloads" <url1> [url2 ...]
```

For link extraction from a video's description (timeline links -> individual tracks):

```bash
bash scripts/download_mp3.sh --extract-from-description <video_url> --out "~/mp3/<folder_name>"
```
...
- Check and install missing dependencies (`yt-dlp`, `ffmpeg`, and `python3` when available) on macOS/Linux.
