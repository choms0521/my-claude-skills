...
```bash
SKILL_DIR="${CODEX_HOME:-$HOME/.codex}/skills/reauth-docmost"
```
...
- Codex는 `DOCMOST_MCP_TOKEN`을 셸 시작 때 읽는다. 새 셸을 열거나
  `source ~/.codex/docmost-token.env`로 다시 읽은 뒤 Codex를 재시작해야 새 토큰이 반영된다.
  이미 떠 있던 세션은 옛 토큰을 물고 있다.
- 재시작 후 docmost 도구를 한 번 호출해 인증이 회복됐는지 확인한다.
