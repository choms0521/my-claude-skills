...
| **check-github-copilot-review** | GitHub Copilot 리뷰 코멘트 자동 처리 — 지적을 무조건 수용하지 않고 코드를 직접 검증해 **실제 결함만 수정**, 취향·오탐 제안은 사유 댓글과 함께 기각, 확정 못 한 항목은 보류(HOLD)하여 사용자에게 확인. 처리 후 resolve + 커밋·푸시, 최대 3사이클 자동 폴링 | `/check-github-copilot-review [PR URL \| PR번호] [--reset]` |
...
| **xmultiplan** | 런타임 교차 합의 계획 — PLANNER(초안)와 CRITIC(비평)을 서로 다른 런타임에 배정해 APPROVE까지 닫힌 루프(최대 N라운드). 기본값 **Codex 초안 + Claude 비평**, `--planner`/`--critic`/`--model`/`--effort`로 역할·모델 가변(`--model`/`--effort` 미지정 시 `~/.codex/config.toml` 기본값 존중). 외부 호출은 `codex exec -s read-only`·`claude -p`, 미가용 시 HOST 인라인 폴백. 다단계(step 2개 이상) 계획에는 step 의존성 그래프(mermaid)와 병렬 실행 Wave 분해를 포함해 집행 주체가 병렬화를 판단할 수 있게 함. 계획 전용 — 결과를 `pending approval`로 `docs/plans/`에 저장하고 실행하지 않음 | `/xmultiplan [--planner codex\|claude] [--critic codex\|claude] [--model M] [--effort minimal\|low\|medium\|high\|xhigh] [--rounds N] <작업>` |
...
# 현재 프로젝트 세션만, 3일로 한정
/agent-supervisor --days 3 --scope current

# 런타임 교차 합의 계획 (기본: Codex 초안 → Claude 비평)
/xmultiplan 결제 모듈에 멱등성 키를 도입하는 계획을 세워줘

# 역할을 뒤집어 Claude 초안 → Codex 비평 (기존 ralplan --critic codex와 동치)
...
agent-supervisor는 다른 스킬과 달리 **심사관 서브에이전트**를 사용합니다. 설치 후 처음 `/agent-supervisor`를 실행할 때 다음이 자동으로 일어납니다.

1. **심사관 자동 설치**: 번들된 `agent-supervisor-judge` 서브에이전트가 `~/.claude/agents/agent-supervisor-judge.md`에 멱등 설치됩니다(관리 마커로 버전 추적). 마커가 없는 파일은 사용자 작성본으로 보고 건드리지 않지만, 관리 마커가 있는 파일은 번들 버전이 바뀌면 파일 전체가 다시 쓰여 수동 편집은 보존되지 않습니다.
2. **재적재 필요(첫 설치 직후 1회)**: Claude Code는 서브에이전트를 세션 시작 시 발견하므로, 갓 설치된 `agent-supervisor-judge`는 같은 세션에서 바로 잡히지 않을 수 있습니다. 이 경우 `/reload-skills`를 실행하거나 세션을 재시작한 뒤 다시 `/agent-supervisor`를 호출하세요.
3. **산출물 위치**: 진단 이력은 `~/.claude/.agent-supervisor/history/`에, 문답으로 합의한 규칙은 `~/.claude/rules/agent-discipline.md`(관리 마커 구획)에 저장됩니다. 위반이 없으면 규칙 파일은 작성하지 않습니다.

> **런타임:** agent-supervisor는 **Claude Code 전용**입니다. 세션 JSONL 기록·서브에이전트·`AskUserQuestion`에 의존하므로 Codex 런타임에서는 동작하지 않습니다.
>
...
Codex/`agy` CLI가 미설치된 경우 Claude만 사용하여 동작합니다.
