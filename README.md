...
| **fe-interview** | 프론트엔드 면접 코치 — Knowledge Graph 기반 3명 면접관, S/A/B/C/D 등급, 합의 평가, 개선 로드맵. 테스트 모드(dry-run/tester agent) 지원 | `/fe-interview [--test dry-run\|agent:junior\|agent:mid\|agent:senior] [--level junior\|mid\|senior] [--length short\|medium\|long]` |
